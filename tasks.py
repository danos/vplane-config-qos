...
# If you specify `all` (eg `invoke flake8 all`) then all files will be checked.
# **************************************************
...
def get_files(commits: str) -> List:
    def get_all_files(repo_root: str) -> List:
        ...
        ignored_files = subprocess.check_output([f"git check-ignore {all_files_str}"], shell=True).decode("utf-8").rstrip().split()
